...
| `RecallCampaign`        | Yes                           | Yes           | Tenant-scoped rows, no-send review states, and `@@index([tenantId, status])`.                                                                     | Same tenant policy as `Patient`; `patientId` may be null for tenant-wide campaigns.                                                 | Optional `patientId` references `Patient.id` only today; future composite FK or invariant checks should prevent cross-tenant patient links. Approval does not send.         |
